...
export default function MainStage({ visible }: { visible: boolean }) {
  ...
  /* Re-centre the ticker when the container width changes (window resize, or the
     Melvin AI panel docking/undocking which shifts the shell without a resize). */
  useEffect(() => {
  ...
  return (
    <>
      ...
      >
        ...
        {/* ── Nav ── */}
        <nav
          className="min-h-11 shrink-0 grid grid-cols-[auto_1fr] lg:grid-cols-3 items-center gap-3 px-4 py-2 lg:px-8 lg:py-0"
          style={{ borderBottom: `1px solid ${theme === "light" ? "rgba(0,0,0,0.15)" : "rgba(255,255,255,0.14)"}` }}
          onMouseEnter={() => setCursorMode("default")}
        >
          <div className="hidden lg:flex items-center gap-3 text-[10px] tracking-[0.25em] uppercase"
          ...
          <div className="flex flex-wrap justify-end gap-x-4 sm:gap-x-5 gap-y-2 text-[10px] tracking-[0.2em] sm:tracking-[0.25em] uppercase">
            {[
